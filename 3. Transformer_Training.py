...
from data_preprocessing_revise import data_processing, split_train_test, input_target,input_target_grouping_1d
from model import CustomDataset,TransformerModel
...
for fp in ['lead','rear']:
    ...
    for fold, (t, v) in enumerate(kfolds.split(train)):
        ...
        model = TransformerModel(in_dim=108,d_model=128,nhead=4,num_layers=4,dim_feedforward=128,dropout=0.2,out_dim=3).to(device)
        ...
        for epoch in range(epochs):
            ...
            if best_loss > val_loss:
                ...
                torch.save(model, f"model/Transformer1-re-{fp}-best-model.pt")
                torch.save(model.state_dict(), f'model/Transformer1-re-{fp}-best-model-parameters.pt')
